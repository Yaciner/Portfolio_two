...
class Loader {
	...
	setupDebug() {
		this.isDebug = window.location.hash.indexOf('debug') > 0;
		this.isGrid = window.location.hash.indexOf('grid') > 0;
		this.isOrbit = window.location.hash.indexOf('orbit') > 0;
	...
	onDebugButtonClick(e) {
		...
		if(this.isDebug) {
			...
			window.location.href = `${baseURL}#debug`;
		}
